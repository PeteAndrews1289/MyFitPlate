...
| Food trust | Documented Trust Score model v2; independent-source allowlist; checksum-valid GTIN and exact USDA product matching; strict evidence invalidation after edits; bounded cross-check latency; finite-value, macro, serving, fat, and mineral sanity guards; separate estimate/review/correction states; model-versioned privacy-safe telemetry; AI-estimate refine; multi-database search; micronutrient ingestion fixes |
...
_Latest Trust-model verification: 894 Core tests, all 76 XCTest app tests, the Swift Testing app test, and the focused first-tap Trust Hub UI flow passed with zero failures. The complete UI suite from the preceding accessibility pass also remains green. The final Debug app build passed with 217 Swift files lint-clean; strict SwiftLint, privacy-manifest validation, and `git diff --check` are green. Trust detail was visually checked in light mode, true dark mode, and the largest accessibility text size on iPhone 17e. The final Trust gallery source was recaptured and both 1320x2868 and 1284x2778 compositions were visually approved; all 16 gallery dimensions were revalidated. GitHub's clean generic-device Release build remains the authoritative pre-merge archive gate._
...
2. **Deploy Firestore rules.** This Trust hardening changes `firestore.rules`; run `firebase deploy --only firestore:rules` from the repository root and confirm the deploy targets the production MyFitPlate project. No callable Function changed in this pass.
3. **Configure production keys/services.** Put the dedicated USDA FoodData Central key in ignored `secrets.xcconfig`; deploy any still-pending Firebase Functions; register App Attest and the development debug token in Firebase App Check. Leave callable enforcement off until App Check metrics show production clients are ready.
4. **App Store Connect.** Set the Privacy Policy and Support URLs, reconcile privacy labels with `PrivacyInfo.xcprivacy`, review export-compliance answers, confirm account deletion instructions, and paste/review the prepared subtitle, promotional text, keywords, and description from `docs/app-store-metadata-2.2.md`.
5. **Watch Gemini key — ask the contributor to revoke.** Every Gemini reference is gone from the codebase (all AI runs through Firebase Functions + the OpenAI server secret); the only exposure is the contributor's own Google quota. Have them delete the key in their AI Studio.
...
7. **Device pass** — run `docs/device-test-2.2.md` end to end. Highest-value: AI consent/Health toggle, account deletion with a throwaway account, record a run (GPS + Live Activity + Health round-trip), a full Beat-the-buffet session, MFP import idempotency, Fast Food Builder, and the Trust scenarios below.
8. Confirm version/build (2.2 / 1, or higher if a 2.2 build was already uploaded) still aligned across app + extensions after any Xcode changes, then validate the signed archive.

## Post-release checklist
1. Keep `feature_communityBarcodeCorrections` set to `false`. Do not enable it until the server-owned aggregation, contributor-privacy, moderation, and conflict-resolution gates in `docs/trust-score-model.md` are complete.
2. Watch Firebase → Analytics: activation funnel (including `elapsed_seconds` and `nutrition_training_loop_completed`), `weekly_recap_viewed`, `mfp_import_completed`, `food_data_suspicious`, `food_correction_action`, Trust model version/source outcomes, share-open events, feedback-email opens, and review-request moments.
