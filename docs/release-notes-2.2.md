...
| Design | DESIGN.md system; new deep-green MFP icon (iOS 18 dark/tinted) + tuned one-family palette; app-wide restructure (Home, Food Search, quick-log, weight, Reports, Meal Plan, Maia, workout player); motion layer; centered labeled Quick Log above five equal-width destinations; scrollable expanded actions on compact phones; final Train target, builder tray, Reports chart-label, Settings goal-method, and compact-device polish |
...
_Full commit history on `main` from the 2.1 archive point. Latest local verification: Core 858 tests and app unit target 76 XCTest tests plus one Swift Testing test passed with 0 failures; focused Quick Log, Food Search, five-tab navigation, and Settings feedback/share UI smoke tests passed. Final Quick Log centering and full expanded-menu reachability also passed on both iPhone 17 Pro Max and compact iPhone 17e simulators. Strict SwiftLint, privacy-manifest validation, and an unsigned generic-device Release build are green._
