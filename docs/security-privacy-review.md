...
Updated for the final 2.3 candidate on July 17, 2026. Version 2.2 is the current public release;
this review describes committed candidate `39e3d1a2` and must remain paired with that exact signed
archive and App Store build.
...
- Recipe URL import fetches only a user-selected public HTTPS page on device, extracts bounded
  page text, then sends that isolated text through the authenticated AI path for structured review.
...
- Recipe URL text is JSON-encoded inside an explicit untrusted-data boundary. A full recipe is
  accepted only when the model affirmatively classifies the page as a recipe; missing or negative
  classification remains a non-recipe result instead of retrying into a more permissive parse.
...
- Account-sensitive caches use a SHA-256-derived account scope, services clear their in-memory
  state when the active account changes, and asynchronous completions reject results if the
  requesting account is no longer current. Sign-out clears online speech and ecosystem context;
  account deletion additionally clears the entire local defaults domain.
...
- Public recipe imports require HTTPS, reject local/private/link-local/reserved literal addresses,
  revalidate every redirect, use an ephemeral cookie-free/cache-free session, set a 20-second
  request timeout and byte range, stream at most 2 MiB, and require a successful final HTTPS HTTP
  response with an HTML-compatible MIME type and UTF-8 content.
- USDA and Open Food Facts use ephemeral cookie-free/cache-free sessions, bounded normalized
  barcode/search inputs, JSON accept headers, and successful HTTP status checks. Open Food Facts
  also sends a versioned identifying User-Agent and builds search parameters with `URLQueryItem`
  so user text cannot inject provider options.
...
- Firestore Rules require every supported owner alias to be nonempty, mutually consistent, and
  equal to the authenticated UID. Posts, groups, and memberships cannot transfer or rewrite the
  original owner through a legacy alias.
- Shipping logs contain no `print`/`debugPrint` calls. HealthKit weight values and dates are
  explicitly private in unified logging, callable failures log bounded error metadata rather than
  raw provider errors, and analytics continues to reject nutrition, body, workout, location, free-
  form, barcode, and account-detail parameters.
- The dormant Firestore sample seeder was removed from the shipping target. AddressSanitizer and
  ThreadSanitizer both pass the complete 1,220-test Core matrix; the sanitizer pass found and fixed
  concurrent test-recorder access before the final clean reruns. Those recorders are DEBUG-only and
  do not enter the Release product.

## Release Owner Actions

1. [x] Publish the reviewed privacy policy, terms, and support pages on public `main` URLs.
2. [x] Deploy the Health Canada, NIH, purpose-routed camera, Maia speech, account deletion, and
   private community-support Functions from backend commit `0aa44de9`. The current API project does
   not expose the preferred speech or GPT-5.6 vision models, so production correctly uses the
   documented compatibility behavior.
3. [x] Keep public community barcode reads disabled and server configuration fail-closed. The
   backup-first public rollout in `docs/community-barcode-consensus-2.3.md` remains post-release.
4. [x] Validate account deletion and private contribution cleanup with throwaway accounts.
5. [x] Register App Attest. Keep callable enforcement off until the uploaded signed/TestFlight build
   produces consistently valid App Check metrics.
6. [x] Supply the USDA FoodData Central key through ignored `secrets.xcconfig`. Candidate
   `39e3d1a2` explicitly substitutes it into the processed app Info.plist, and package inspection
   verifies that OpenAI and FatSecret credentials remain absent. Move USDA behind an authenticated
   backend before provider usage or quota becomes material.
7. [x] Reconcile the 13 App Store Connect data types with the first-party manifests and public
   policy. Recheck the published preview once more when selecting the final build.
8. [ ] Have qualified counsel review the policy and terms before relying on them as legal advice.
...
- The current public-recipe host policy blocks private literal addresses and revalidates redirects,
  but an on-device URL session cannot fully prevent a hostile public hostname from changing DNS
  resolution after validation. Keep imported HTML untrusted and bounded, as the current parser does.
- The USDA FoodData Central key is stored only in ignored local build configuration and substituted
  into the signed app at build time. Any key compiled into an iOS app can be extracted, so provider-
  side quota/restriction and an authenticated backend remain required before meaningful scale.
...
- Peter physically accepted HealthKit, route recording, background behavior, Watch sync, account
  deletion, permissions, accessibility, and the other device journeys in sections 1-9 of
  `docs/physical-acceptance-2.3.md`. App Store processing, TestFlight App Check validity, and
  post-release production behavior remain external evidence rather than source-test evidence.
