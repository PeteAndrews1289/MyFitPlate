...
class RoutineEditorViewModel: ObservableObject {
    ...
    @Published var exerciseToEdit: RoutineExercise?
    ...
    init(routine: WorkoutRoutine, onSave: @escaping (WorkoutRoutine) -> Void) {
    ...
    func applyTemplate(_ template: RoutineEditorTemplate) {
    ...
    func addExercise(from draft: ExercisePickerDraft) {
        exercises.append(makeExercise(name: draft.name, category: draft.category, type: draft.type))
        HapticManager.instance.feedback(.light)
    }

    func duplicateExercise(_ exercise: RoutineExercise) {
    ...
    func deleteExercise(_ exercise: RoutineExercise) {
        exercises.removeAll { $0.id == exercise.id }
    }

    func moveExercise(_ exercise: RoutineExercise, direction: RoutineMoveDirection) {
        guard let index = exercises.firstIndex(where: { $0.id == exercise.id }) else { return }
        let targetIndex: Int
        switch direction {
        case .up:
    ...
    func updateExercise(_ updatedExercise: RoutineExercise) {
